...
boot_alloc(uint32_t n)
{
	...
	if((uint32_t)(result - KERNBASE + n) > (uint32_t)(npages * PGSIZE)){
		panic("boot alloc: out of physical memory!\n");	
	}
	nextfree = result + n;
	return result;
}
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	size_t iostart = ROUNDDOWN(IOPHYSMEM, PGSIZE) / PGSIZE;
	size_t exend = PADDR(boot_alloc(0)) / PGSIZE;
	size_t i_MPENTRY_PADDR = MPENTRY_PADDR / PGSIZE;
	page_free_list = NULL;
	for (i = 0; i < npages; i++) {
		pages[i].pp_ref = 0;
		if(i == 0 || (iostart <= i && i < exend) || i == i_MPENTRY_PADDR) {
			continue;
		}
